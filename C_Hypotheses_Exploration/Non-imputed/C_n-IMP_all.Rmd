---
title: "C_all analyses ran with non-Imputed data"
...
```{r}
# imputed data
analyze_data <- read_csv("/Users/Pinocchio/R/Thesis_git_repository/Data_analysis/Data/C_n-Imp_scored.csv")
```
...
```{r}
summary(Mod.Med.fit.csc, fit.measures = TRUE, rsquare = TRUE)
```
...
write.csv(cov_table , "C_n-imp_neg_cov.csv", row.names = T) # Save covariance matrix
```
...
write.csv(cov_table , "C_n-imp_pos_cov.csv", row.names = T) # Save covariance matrix
```
...
write.csv(cov_table , "C_n-imp_nc_cov.csv", row.names = T) # Save covariance matrix
```
...
write.csv(cov_table , "C_n-imp_cc_cov.csv", row.names = T) # Save covariance matrix
```
...
write.csv(cov_table , "C_n-imp_csc_cov.csv", row.names = T) # Save covariance matrix
```
