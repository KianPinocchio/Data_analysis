...
Mi =~ MCQ2 + MCQ4 + MCQ9 + MCQ11 + MCQ15 + MCQ21
...
```{r Fir the latent model}
# Fit the model
Mod.Med.SEM.Latent <- sem(model = MOD.MED.model.Latent,
                   data = analyze_data,
                   se = "bootstrap",
                   test = "bootstrap",
...
```{r}
# Model summary
summary(Mod.Med.SEM.Latent, fit.measures = FALSE, standardize = TRUE, rsquare = TRUE)
```

```{r}
# Parameters table
parameterEstimates(Mod.Med.SEM.Latent, remove.nonfree = TRUE, ci = TRUE, 
                   boot.ci.type = "perc", pvalue = TRUE, output = "pretty")
```

# Standardized parameters
```{r}
std_sols <- standardizedsolution(Mod.Med.SEM.Latent)
std_sols
```

# Paths plot: C1 Y ~ sum_neg
```{r}
# Plot model
plot = semPaths(Mod.Med.SEM.Latent, fixedStyle = 1, layout = "tree2", whatLabels = "est",
                  intercepts = F, label.scale=T, nCharNodes = 5, 
                  sizeMan2=3, sizeMan=6, asize=2, residuals = F, exoCov = F)
```

```{r}
vcov(Mod.Med.SEM.Latent)
```
