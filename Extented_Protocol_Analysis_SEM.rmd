...
"
NEG_c ~ a*CRA_c
sum_cesd ~ c1*CRA_c+c2*SES_c+c3*BSM_c+c4*CRA_c:SES_c+c5*CRA_c:BSM_c+c6*SES_c:BSM_c+c7*interaction1+b1*NEG_c+b2*SES_c+b3*BSM_c+b4*NEG_c:SES_c+b5*NEG_c:BSM_c+b6*SES_c:BSM_c+b7*interaction0 + g*PSS_c
SES_c ~ SES_c.mean*1
SES_c ~~ SES_c.var*SES_c
BSM_c ~ BSM_c.mean*1
BSM_c ~~ BSM_c.var*BSM_c
indirect :=(a)*(b1+b4*SES_c.mean+b5*BSM_c.mean)
direct :=c1+c4*SES_c.mean+c5*BSM_c.mean
total := direct + indirect
prop.mediated := indirect / total
indirect.below :=(a)*(b1+b4*(SES_c.mean-sqrt(SES_c.var))+b5*(BSM_c.mean-sqrt(BSM_c.var)))
indirect.above :=(a)*(b1+b4*(SES_c.mean+sqrt(SES_c.var))+b5*(BSM_c.mean+sqrt(BSM_c.var)))
direct.below:=c1+c4*(SES_c.mean-sqrt(SES_c.var))+c5*(BSM_c.mean-sqrt(BSM_c.var))
direct.above:=c1+c4*(SES_c.mean+sqrt(SES_c.var))+c5*(BSM_c.mean+sqrt(BSM_c.var))
