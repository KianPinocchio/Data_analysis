...
all_columns <- cbind(all_columns, demographics)
```
...
mahal <- mahalanobis(all_columns[ , -c(63:69)],
  colMeans(all_columns[ , -c(63:69)], na.rm=TRUE),
  cov(all_columns[ , -c(63:69)], use ="pairwise.complete.obs"))

cutoff <- qchisq(p = 1 - .001, #1 minus alpha
                 df = ncol(all_columns[ , -c(63:69)])) # number of columns
```
...
cutoff
summary(mahal < cutoff) #notice the direction 
no_outliers1 <- subset(scored_data, mahal < cutoff)
```
...
```{r}
library(corrplot)
corrplot(cor(no_outliers1[ , -c(1:3)]))
```

## Assumptions Set Up

```{r}
random_variable <- rchisq(nrow(no_outliers1), 7)
fake_model <- lm(random_variable ~ ., # no_outliers1$sum_cesd
                 data = no_outliers1[ , -c(1:3)])
