...
```{r Imputation, message=TRUE, include=FALSE}
...
temp_no_outlier <- subset(imputed_data, mahal < cutoff)

summary(temp_no_outlier)
```


# Prepare to score the data with temporarily no outliers

Create name list for MCQ scale
```{r}
...
function_score_data <-function(temp_no_outlier){

#save variables in separate data frame
scored_data <- temp_no_outlier %>%
...
# Score the data with temporarily no outliers
```{r Score data}
scored_data <- function_score_data(temp_no_outlier) #score data first, to find outliers among scored data
summary(scored_data)
```

Detect outliers based on *scored* data
```{r}
# Mahalanobis for scored data
mahal <- mahalanobis(scored_data[ , -c(1:69)],
  colMeans(scored_data[ , -c(1:69)], na.rm=TRUE),
  cov(scored_data[ , -c(1:69)], use ="pairwise.complete.obs"), , tol = 1e-20)

cutoff <- qchisq(p = 1 - .001, #1 minus alpha
                 df = ncol(scored_data[ , -c(1:69)])) # number of columns
```

Drop all outliers detected based on *scored* data
```{r}
# Exclusing outliers among scored data based on Mahalanobis
cutoff
summary(mahal < cutoff) #notice the direction 
no_outlier <- subset(scored_data, mahal < cutoff)
```
...
```{r}
library(corrplot)
plot_data <- cbind(final_scored_data$SES,final_scored_data[ , -c(1:69)])
...
```{r}
sum(duplicated(final_scored_data))
```
# Data summary
```{r}
summary(final_scored_data)
```

# Check demographics
```{r}
final_scored_data$gender <- factor(final_scored_data$gender, levels=c(1,2,3,4)) # Factor gender
summary (final_scored_data$gender)
```
...
```{r}
descriptives <- final_scored_data %>% 
...
```{r}
mod.mod.model = lm(Y ~ X1 + X + W + Z + X*W  + X*Z + W*Z + X*W*Z, data = data)
```
...
```{r}
interactions::sim_slopes(mod.mod.model, pred = X, modx=W, mod2 = Z, modxvals = NULL, 
                         jnalpha = 0.05, digits = 3, n.sd = 1)
```

# Plot simple slopes for interaction
```{r}
interactions::interact_plot(mod.mod.model, pred = X, modx = W, mod2 = Z, centered = "none", 
