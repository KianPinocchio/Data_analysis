...
pacman::p_load(pacman, tidyverse, kableExtra, psych, lavaan, corrplot, performance, MVN, ICS,
               semPlot, gridExtra,knitr, readxl, papaja, report, mice, VIM, rmarkdown, janitor)
```
...
imputed_data <- function_data_imputation(replace_columns, no_columns)
```
...
```{r}
mice_plot <- aggr(imputed_data,numbers=TRUE, sortVars=TRUE,
...
demographics <- subset(imputed_data, select = columns) 
imputed_data <- imputed_data[ , ! names(imputed_data) %in% columns]
imputed_data <- cbind(demographics, imputed_data) # Save all clean data
```
